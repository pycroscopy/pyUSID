...
def make_linear_alpha_cmap(name, solid_color, normalization_val, min_alpha=0, max_alpha=1):
    ...
    if not isinstance(name, (str, unicode)):
        raise TypeError('name should be a string')
    if not isinstance(solid_color, (list, tuple, np.ndarray, dask.array)):
...
def rainbow_plot(axis, x_vec, y_vec, num_steps=32, **kwargs):
    ...
    if not isinstance(axis, mpl.axes.Axes):
        raise TypeError('axis must be a matplotlib.axes.Axes object')
    if not isinstance(x_vec, (list, tuple, np.ndarray, dask.array)):
...
def plot_line_family(axis, x_vec, line_family, line_names=None, label_prefix='', label_suffix='',
    ...
    if not isinstance(line_family, np.ndarray, dask.array):
...
def plot_map(axis, img, show_xy_ticks=True, show_cbar=True, x_vec=None, y_vec=None,
    ...
    if not isinstance(axis, mpl.axes.Axes):
        raise TypeError('axis must be a matplotlib.axes.Axes object')
    if not isinstance(img, np.ndarray, dask.array):
    ...
    def set_ticks_for_axis(tick_vals, is_x):
        ...
        if tick_vals is not None:
            if isinstance(tick_vals, (int, float)):
                ...
                if not isinstance(tick_vals, (np.ndarray, list, tuple, range, dask.array)) or len(tick_vals) != img_size:
                    raise ValueError(
...
def plot_curves(excit_wfms, datasets, line_colors=[], dataset_names=[], evenly_spaced=True,
    ...
    for var, var_name, dim_size in zip([datasets, excit_wfms], ['datasets', 'excit_wfms'], [2, 1]):
        ...
        if isinstance(var, (h5py.Dataset, np.ndarray, dask.array)):
            if not len(var.shape) == dim_size:
                raise ValueError(mesg)
        elif isinstance(var, (list, tuple)):
            if not np.all([isinstance(dset, (h5py.Dataset, np.ndarray, dask.array)) for dset in datasets]):
    ...
    if isinstance(datasets, (h5py.Dataset, np.ndarray, dask.array)):
        # can be numpy array or h5py.dataset
        num_pos = datasets.shape[0]
        num_points = datasets.shape[1]
        datasets = [datasets]
        if isinstance(excit_wfms, (np.ndarray, h5py.Dataset, dask.array)):
        ...
        for dataset in datasets:
            if not isinstance(dataset, (h5py.Dataset, np.ndarray, dask.array)):
...
def plot_complex_spectra(map_stack, x_vec=None, num_comps=4, title=None, x_label='', y_label='', evenly_spaced=True,
    ...
    **kwargs will be passed on either to plot_map() or pyplot.plot()
    ...
    fig, axes
    """
    if not isinstance(map_stack, np.ndarray, dask.array) or not map_stack.ndim in [2, 3]:
...
def plot_scree(scree, title='Scree', **kwargs):
    ...
    if not (isinstance(scree, np.ndarray, dask.array) or isinstance(scree, h5py.Dataset)):
...
def plot_map_stack(map_stack, num_comps=9, stdevs=2, color_bar_mode=None, evenly_spaced=False, reverse_dims=False,
    ...
    fig, axes
    """
    if not isinstance(map_stack, np.ndarray, dask.array) or not map_stack.ndim == 3:
    ...
    for var, var_name in zip([fig_mult, pad_mult], ['fig_mult', 'pad_mult']):
        if not isinstance(var, (list, tuple, np.ndarray, dask.array)) or len(var) != 2:
